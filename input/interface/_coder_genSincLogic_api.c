/*
 * File: _coder_genSincLogic_api.c
 *
                      ...
                      const mxArray *plhs[4])
{
  ...
  emxInit_real_T(&st, &t);
  emxInit_real_T(&st, &x);
  genSincLogic(A, f, duration, Fs, t, x, &samples, &cycles);
  ...
  emxFree_real_T(&st, &x);
  if (nlhs > 2) {
    plhs[2] = b_emlrt_marshallOut(samples);
  }
  if (nlhs > 3) {
    plhs[3] = b_emlrt_marshallOut(cycles);
  }
...
void genSincLogic_atexit(void)
{
  ...
  genSincLogic_xil_terminate();
  genSincLogic_xil_shutdown();
  emlrtExitTimeCleanup(&emlrtContextGlobal);
}
...
/*
 * File trailer for _coder_genSincLogic_api.c
 *
 * [EOF]
 */
